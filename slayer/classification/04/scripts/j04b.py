...
encoder = FeatureEncoderArray([
    SpikeEncoder(dmin=0, dmax=1, time_steps=time_steps) for _ in range(39)
])
decoder = VoteDecoder([0, 1])

slayer_net = SlayerRispNetwork(
    threshold=0.2942629262347669,
    decay="all",
    discrete=True,
    min_potential=-0.011826305698227466,
    threshold_inclusive=False,
    encoder=None,
    label_encoder=None,
    decoder=decoder,
    delay_bounds=(1.0159590951499489, 18.13789285427414),
    persistent_state=False,
    pre_hook_fx=quantize_8bit,
    tau_grad=0.4915290154195714,
    scale_grad=1.9927746214904067
)
...
loss = loss_func(true_rate=0.8757759649994138, false_rate=0.07268366009796497)
...
stats = train_net(
    ...
    batch_size=300,
    num_workers=2,
    optimizer=torch.optim.Adam(params=slayer_net.parameters(), lr=0.007018442965284264),
    loss_func=loss,
    pred_func=pred_func,
    verbose=True,
    sim_time=50
...
with open("fred_dense_timeseries.json", 'w') as f:
